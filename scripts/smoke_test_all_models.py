        ...
        if param_schema.get('required', False):
            ...
            if param_type == 'string':
                ...
                if item_type == 'string':
                    ...
                    if 'url' in param_name.lower() or 'image' in param_name.lower() or 'video' in param_name.lower() or 'audio' in param_name.lower():
                        params[param_name] = [f"https://example.com/test{file_ext}"]
                    else:
...
async def test_model_async(model_id: str, model_type: str) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        if not schema:
            return ('fail', f"Schema not found in YAML")
        ...
        # Timeout
        final_status = status_result.get('status') or status_result.get('state', 'unknown') if status_result else 'unknown'
