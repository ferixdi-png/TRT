...
## 2026-02-16: Эмодзи в кнопках главного меню

### Было → стало
- **Было:** главное меню показывало кнопки без эмодзи (например, “БЕСПЛАТНЫЕ МОДЕЛИ”, “Баланс”).  
  **Стало:** добавлены эмодзи к видимым подписи кнопок (например, “🆓 БЕСПЛАТНЫЕ МОДЕЛИ”, “💳 Баланс”), callback_data не менялись.

### Изменённые файлы
- `helpers.py`
- `tests/test_main_menu.py`
- `tests/test_menu_anchor.py`
- `TRT_REPORT.md`

### Как проверял
- `pytest -q tests/test_main_menu.py tests/test_menu_anchor.py`
## 2026-03-10: Аудит лагов + UX “🧩 Другие модели”

### Лаги/UX — что нашли
- **DB-lock:** в проде возможен file_fallback при отсутствии `DATABASE_URL`, что допускает второй экземпляр и гонки.
- **GitHubStorage hot reads:** часто читаемые JSON-файлы не отличались по TTL, из-за чего повторные чтения дергали GitHub чаще, чем нужно.
- **Handler latency:** нет единых per-handler логов времени обработки (сложно ловить хвосты).
- **“🧩 Другие модели”:** переход не обновлял контекст сессии как остальные кнопки, не было дефолтного “быстрого старта” на `sora-watermark-remover`.

### Сделано
- **DB-lock:** переключение на DB lock при наличии `DATABASE_URL`, в проде запрещен file_fallback.  
- **GitHubStorage:** добавлен отдельный TTL для hot-файлов (балансы/лимиты), чтобы уменьшить горячие чтения.  
- **Latency logs:** добавлены per-handler логи времени обработки (start, callbacks, input params, global routers).  
- **“Другие модели”:** выровнен session flow, добавлена дефолтная кнопка на `sora-watermark-remover` вверху списка.
