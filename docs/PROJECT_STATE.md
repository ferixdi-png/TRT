...
- Iteration 22–23: re-audited merge conflicts (no markers present, origin remote unavailable), reran callback/history/payload suites to confirm readiness; branch remains clean for PR #9 recreation
- Iteration 24: added repo scan guard to fail on merge markers/branch-garbage in code/docs/tests
...
- [ ] G7 History — callback persistence added; history UI now reads DB jobs via async helper with task_id surfaced; success/fail display still needs verification

## 3) FREE TOP-5 status matrix
| Model | payload ok? | wizard ok? | createTask ok? | poll/callback ok? | media send ok? | history ok? |
|-------|-------------|------------|----------------|-------------------|----------------|-------------|
| z-image | ✅ contract tests (adds default aspect_ratio + no-overlay guard) | ✅ wizard fallback injects aspect_ratio even without overlay | ✅ aspect_ratio auto-filled even when empty | ☐ (progress awaited; callback persistence wired; reply-once guard added) | ☐ | ☐ (history now DB-backed with task_id, needs e2e proof) |
...
|-------|--------|-------------------|--------------------|---------------------------|-------------|---------------------|-------------|
| google/imagen4-fast | ✅ | ✅ (schema-based) | ✅ | ✅ (aspect_ratio/num_images/seed/negative_prompt exposed) | ✅ | ☐ | ☐ (DB-backed list ready) |
...
- “История” → history handler — value: recall past jobs; now fetches DB-backed jobs (async) including task_id; needs success/fail coverage
...
- Persist callback outcomes into history records (succeeded/failed) and prove reply-once covers poll-first vs callback-first flows end-to-end (DB-backed history now available).
