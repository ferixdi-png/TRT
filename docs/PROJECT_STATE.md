...
- [ ] G7 History — callback persistence added; history UI now reads DB jobs via async helper; success/fail display still needs verification

## 3) FREE TOP-5 status matrix
| Model | payload ok? | wizard ok? | createTask ok? | poll/callback ok? | media send ok? | history ok? |
|-------|-------------|------------|----------------|-------------------|----------------|-------------|
| z-image | ✅ contract tests (adds default aspect_ratio + no-overlay guard) | ✅ wizard fallback injects aspect_ratio even without overlay | ✅ aspect_ratio auto-filled even when empty | ☐ (progress awaited; callback persistence wired; reply-once guard added) | ☐ | ☐ (history now DB-backed, needs e2e proof) |
...
- “История” → history handler — value: recall past jobs; now fetches DB-backed jobs (async); needs success/fail coverage
