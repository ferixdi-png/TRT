...
### Диагностика пропажи файла после генерации (KIE)

Если задача в KIE создаётся, но пользователю не приходит файл, чаще всего проблема
происходит между polling → download → send. Ниже чеклист, который можно пройти по логам:

1. **Фиксация идентификаторов**
   - В логах должен быть сохранён `task_id` и связанный `job_id`.
2. **Проверка статусов**
   - Статусы должны пройти цепочку `queued` → `running` → `succeeded`.
3. **Наличие результата**
   - В ответе на `succeeded` должны присутствовать `result_url` или `bytes`.
4. **Отправка файла ботом**
   - Бот должен зафиксировать `file_id` и финальный статус отправки пользователю.
5. **Исключения и retry**
   - Любое исключение в цепочке должно логироваться и сопровождаться
     читаемым сообщением + кнопкой «Повторить», чтобы пользователи могли
     стабильно завершать генерацию даже при нагрузке.

Этот чеклист помогает быстро локализовать сбой и понять, в каком именно шаге
теряется результат задачи.
