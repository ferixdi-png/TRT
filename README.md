...
## 🔄 Обновление моделей/параметров/цен (Kie Sync)

**ВАЖНО:** Все изменения моделей, параметров и цен должны проходить через безопасный процесс синхронизации.

### Безопасное обновление через Kie Sync

Kie Sync - инструмент для безопасной синхронизации данных из документации Kie.ai с локальным SOURCE_OF_TRUTH без нарушения существующих контрактов.

**Команды:**

```bash
# 1. Получить свежие данные из Kie.ai (локально, с сетью)
make kie-sync-pull
# или
python scripts/kie_sync.py pull

# 2. Построить нормализованный upstream JSON (из кеша или сети)
make kie-sync-build
# или
python scripts/kie_sync.py build

# 3. Сверка с локальным реестром (dry-run, без изменений)
make kie-sync-reconcile
# или
python scripts/kie_sync.py reconcile --no-write

# 4. Применить изменения (после проверки diff)
python scripts/kie_sync.py reconcile
```

**Гарантии безопасности:**

- ✅ **Никогда не удаляет** существующие поля из локального реестра
- ✅ **Никогда не меняет** required→optional или optional→required автоматически
- ✅ **Новые поля** добавляются как `experimental=true` и `optional`
- ✅ **Новые модели** добавляются как `disabled=true` (требуют ручного включения)
- ✅ **Цены** рассчитываются строго по формуле: `RUB = USD * 78 * 2`
- ✅ **Dry-run режим** (`--no-write`) для проверки изменений перед применением

**Формула ценообразования (фиксированная):**
...
Где:
- `USD` - цена из документации Kie.ai
- `78` - фиксированный курс USD→RUB (можно переопределить через `USD_RUB_RATE` ENV)
- `2` - фиксированный множитель наценки (никогда не меняется)

**Пример workflow:**

```bash
# 1. Проверить что изменилось (dry-run)
python scripts/kie_sync.py reconcile --no-write

# 2. Если diff приемлем - применить
python scripts/kie_sync.py reconcile
...
- `make kie-sync` - запускает build + reconcile на фикстурах (без сети)
- Тесты парсера: `python -m pytest tests/test_kie_parser.py -v`
