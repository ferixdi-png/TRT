...
# NOTE: WEBHOOK_BASE_URL is conditionally required (only if BOT_MODE=webhook)
# It's checked separately in validate_webhook_requirements()
REQUIRED_ENV_KEYS = [
    'ADMIN_ID',
    'BOT_MODE',
    'KIE_API_KEY',
    ...
    'SUPPORT_TEXT',
    'TELEGRAM_BOT_TOKEN',
    # WEBHOOK_BASE_URL removed - checked conditionally in validate_webhook_requirements()
]
...
def validate_env_keys() -> Tuple[bool, List[str]]:
    ...
    NOTE: WEBHOOK_BASE_URL is NOT checked here - it's conditionally required
    only if BOT_MODE=webhook, and is checked in validate_webhook_requirements().
