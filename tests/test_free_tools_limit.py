...
async def test_hourly_limit_resets(monkeypatch):
    ...
    for _ in range(5):
        result = await free_tools_service.check_and_consume_free_generation(123, model_id)
        assert result["status"] == "ok"

    denied = await free_tools_service.check_and_consume_free_generation(123, model_id)
...
async def test_referral_bonus_adds_capacity(monkeypatch):
    ...
    for _ in range(5):
        result = await free_tools_service.check_and_consume_free_generation(456, model_id)
        assert result["status"] == "ok"

    referral_use = await free_tools_service.check_and_consume_free_generation(456, model_id)
