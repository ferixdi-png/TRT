...
def _make_session(monkeypatch, head_response, get_response):
    def factory(*args, **kwargs):
        return DummySession(lambda: head_response, lambda: get_response)
...
def test_deliver_result_url_ok(monkeypatch):
    ...
    _make_session(monkeypatch, head_response, get_response)
...
def test_deliver_result_multi_urls_group_and_sequential(monkeypatch):
    ...
    audio_head = DummyResponse(headers={"Content-Type": "audio/mpeg"}, content_length=1024)
    audio_get = DummyResponse(headers={"Content-Type": "audio/mpeg"}, body=b"data")
    _make_session(monkeypatch, audio_head, audio_get)
