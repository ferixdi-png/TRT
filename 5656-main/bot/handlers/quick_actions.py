...
async def show_quick_actions(callback: CallbackQuery, state: FSMContext, cid=None, bot_state=None):
    ...
    if not callback.from_user:
        logger.error("[QUICK_ACTIONS] callback.from_user is None in show_quick_actions")
    ...
    if not callback.message:
        logger.error("[QUICK_ACTIONS] callback.message is None in show_quick_actions")
...
async def show_action_details(callback: CallbackQuery, state: FSMContext):
    ...
    # CRITICAL: None check for callback.from_user
    if not callback.from_user:
        logger.error("[QUICK_ACTIONS] callback.from_user is None in show_action_details")
        await callback.answer("❌ Ошибка: не удалось определить пользователя.", show_alert=True)
        return
    
    # CRITICAL: None check for callback.message
    if not callback.message:
        logger.error("[QUICK_ACTIONS] callback.message is None in show_action_details")
        await callback.answer("❌ Ошибка: сообщение недоступно.", show_alert=True)
        return
...
async def show_action_examples(callback: CallbackQuery, state: FSMContext):
    ...
    # CRITICAL: None check for callback.from_user
    if not callback.from_user:
        logger.error("[QUICK_ACTIONS] callback.from_user is None in show_action_examples")
        await callback.answer("❌ Ошибка: не удалось определить пользователя.", show_alert=True)
        return
    
    # CRITICAL: None check for callback.message
    if not callback.message:
        logger.error("[QUICK_ACTIONS] callback.message is None in show_action_examples")
        await callback.answer("❌ Ошибка: сообщение недоступно.", show_alert=True)
        return
...
async def use_quick_example(callback: CallbackQuery, state: FSMContext):
    ...
    # CRITICAL: None check for callback.from_user
    if not callback.from_user:
        logger.error("[QUICK_ACTIONS] callback.from_user is None in use_quick_example")
        await callback.answer("❌ Ошибка: не удалось определить пользователя.", show_alert=True)
        return
    
    # CRITICAL: None check for callback.message
    if not callback.message:
        logger.error("[QUICK_ACTIONS] callback.message is None in use_quick_example")
        await callback.answer("❌ Ошибка: сообщение недоступно.", show_alert=True)
        return
