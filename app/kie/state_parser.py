            ...
            try:
                ...
                if isinstance(result_data, dict):
                    ...
                    if isinstance(urls, list):
                        result_urls = [str(url) for url in urls if url]
                    elif isinstance(urls, str):
                        result_urls = [urls]
                
                logger.info(f"{prefix}SUCCESS: {len(result_urls)} URL(s) parsed from resultJson")
            except json.JSONDecodeError as e:
                logger.error(f"{prefix}Failed to parse resultJson: {e}")
                logger.debug(f"{prefix}resultJson value: {result_json_str[:200]}...")
        
        # Fallback: direct resultUrls field
        if not result_urls:
            direct_urls = data.get('resultUrls')
            if isinstance(direct_urls, list):
                result_urls = [str(url) for url in direct_urls if url]
